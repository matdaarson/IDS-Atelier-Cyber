...
def build_autoencoder(input_dim: int, latent_dim: int = None):
    ...
    x = layers.Reshape((input_dim, 1))(encoder_input)

    x = layers.Bidirectional(
            layers.LSTM(input_dim // 2, 
    ...
    # Étendre le latent en séquence
    x = layers.Dense(input_dim // 4)(decoder_input)
    x = layers.LeakyReLU(0.1)(x)

    # On répète le vecteur latent pour reconstruire une séquence
    ...
    # On remet en forme (batch, input_dim)
    ...
    autoencoder = models.Model(encoder_input, autoencoder_output, name="bilstm_autoencoder")
