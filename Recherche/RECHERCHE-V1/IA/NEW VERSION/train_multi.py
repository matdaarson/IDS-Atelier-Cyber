...
def load_all_csv_in_folder(folder_path: str, threshold=0.3):
    ...
    for f in csv_files:
        ...
        # Nettoyage initial du CSV
        df = load_and_clean_csv(f, threshold=threshold)

        # FORCER numeric (évite colonnes objet)
        df = df.apply(pd.to_numeric, errors="coerce")

        # Remplacement de tout inf par NaN
        df = df.replace([np.inf, -np.inf], np.nan)

        dfs.append(df)


    # === Étape 1 : Colonnes communes uniquement ===
    ...
    # Réduction aux colonnes communes
    ...
    # === Étape 3 : Suppression des colonnes constantes ===
    ...
    # Vérification finale
    if np.isnan(df_total.values).any():
